...
|---|---|---|---|
| **MCQ** | Mã cơ quan (phân cấp) | Mã định danh điện tử theo QĐ 20/2020 + QĐ 09/2025, có thể nhiều cấp ngăn cách bởi dấu chấm (xem chi tiết bên dưới) | `G09.01` |
...
#### Cấu trúc phân cấp của MCQ (Mã cơ quan)

Mã định danh điện tử được xây dựng theo cấp bậc, phát triển từ trái sang phải, ngăn cách bởi dấu chấm (`.`). Tối đa 35 ký tự, gồm `A-Z`, `0-9`, `.`

```text
G09           → Mã cấp 1: toàn bộ Bộ Nội vụ
G09.01        → Mã cấp 2: đơn vị trực thuộc (ví dụ: Cục Văn thư và Lưu trữ NN)
G09.01.001    → Mã cấp 3: đơn vị trực thuộc cấp 2 (ví dụ: phòng/ban cụ thể)
```

| Cấp | Ý nghĩa | Ai quy định |
|---|---|---|
| **Cấp 1** (ví dụ `G09`) | Bộ/ngành/tỉnh — do Thủ tướng ban hành (Phụ lục I QĐ 20/2020) | Thủ tướng Chính phủ |
| **Cấp 2** (ví dụ `G09.01`) | Đơn vị trực thuộc cấp 1 (cục, vụ, văn phòng, sở...) | Từng bộ/tỉnh tự xây dựng, đăng ký với Bộ KH&CN |
| **Cấp 3+** (ví dụ `G09.01.001`) | Đơn vị cấp sâu hơn | Từng bộ/tỉnh tự xây dựng |

> **Lưu ý:** Danh sách mã cấp 2, cấp 3 cụ thể của từng bộ/tỉnh **không có trong FRD**. Khi triển khai, cần tra cứu từ hệ thống quản lý Danh mục dùng chung quốc gia (Bộ KH&CN) hoặc nhập thủ công theo từng đơn vị áp dụng. Trong DocCode, dùng mã cấp thấp nhất đặc định đơn vị phát hành văn bản.
